...
valid_users = exp_result %>% filter(course_id=='interactive') %>% group_by(student_id) %>% summarize(n=n()) %>% filter(n==2)
Q1 = exp_result %>% 
  filter(course_id=='interactive', posttest=='similar', student_id %in% valid_users$student_id) %>%
  select(student_id, master) %>% rename(q1=master)
Q2 = exp_result %>% 
  filter(course_id=='interactive', posttest=='transfer', student_id %in% valid_users$student_id) %>%
  select(student_id, master) %>% rename(q2=master)
Q = merge(Q1,Q2) %>% select (-student_id)
