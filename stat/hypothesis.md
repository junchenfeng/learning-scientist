...
# 2.显著性检验（Fisher Significance Test）
...
# 3.假设检验（Neyman-Pearson Hypothesis Test）
...
首先，仅仅有原假设是不够的。在实验前，你必须提出一个具体的备择假设。比如，如果硬币不均匀，头朝上的概率是0.7
...
用同样的逻辑，可以知道N=40是符合参数配置的最小样本量。（二类错误概率大约为0.19）
...
一共抛40次，假设抛出了26（或以上），拒绝H0（8的pvalue低于alpha）；假设抛出了25（或以下），不能拒绝H0
...
# 4.原假设显著性检验（Null Hypothesis Statistical Significance Test）
